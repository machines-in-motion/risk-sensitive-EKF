...
class ViconFrame:
    def __init__(self, pose = np.zeros(7), v_world = np.zeros(6), v_body = np.zeros(6)):
        self.se3_pose = pose
        self.velocity_world_frame = v_world 
        self.velocity_body_frame = v_body 

class ViconClient:
    def __init__(self):
        pass

    def initialize(self, host_name):
        self.host_name = host_name

    def run(self):
        self.listener = QualisysListener(self.host_name)

    def get_vicon_frame(self):
        pos = np.concatenate([self.listener.getPosition(),self.listener.getOrientationQuat()])
        vel_world = np.concatenate([self.listener.getVelocity(),self.listener.getAngularVelocity()])
        rot_mat_inv = np.linalg.inv(self.listener.getOrientationMat9())
        vel_body = np.concatenate([rot_mat_inv.dot(vel_world[:3]), rot_mat_inv.dot(vel_world[3:])])
        return ViconFrame(pos, vel_world, vel_body)

# Taken from solopython
